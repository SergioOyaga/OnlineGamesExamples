package org.soyaga.examples.MadVirus.GA;
...
import org.soyaga.examples.MadVirus.GA.Initializer.MadVirusInitializer;
import org.soyaga.examples.MadVirus.GA.Mutations.RandomColorMutation;
import org.soyaga.examples.MadVirus.GA.StoppingPolicy.TargetFeasibilityCriteriaPolicy;
 ...
 */
public class MadVirusGA extends StatsGeneticAlgorithm {
    private final Graph graph;
    /**
     * Constructor that matches its super constructor.
     *
     * @param ID String with the GA description.
     * @param choices hashset with the color choices.
     * @param turns Integer with the max number of turns.
     * @param graph Graph with the problem relations.
     */
    public MadVirusGA(String ID, ArrayList<Integer> choices, Integer turns, Graph graph) throws IOException {
        super(ID,
                10,
                ...
                new ByLevelLevelProbMutPolicy(
                        new ArrayList<>(){{
                            add(
                                new ArrayList<>(){{
                                    add(new RandomColorMutation(choices));
                                }});
                            }},
                        new ArrayList<>(){{add(0.8);}},
                        false,
                        false
                ),
                new FixedElitismPolicy(1),
                new FixedNewbornPolicy(1),
                new MadVirusInitializer(
                        new MadVirusObjectiveFunction(graph),
                        new MadVirusFeasibilityFunction(graph),
                        choices,
                        turns
                ),
    ...
    public Object getResult(Object... resultArgs) {
        ArrayList<Integer> genomeContainer = (ArrayList<Integer>) this.population.getBestIndividual().getGenome().getGeneticInformation();
        ...
        for(Integer chromosome:genomeContainer){
            ...
            if(actions.isEmpty() || !Objects.equals(actions.get(actions.size() - 1), chromosome)) actions.add(chromosome);
            if(graph.getNodes().size()== reachedNodes.size()) break;
        }
        return new Object [] {"GA_Optimal",  actions};
    }
}
